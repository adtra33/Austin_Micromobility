...
```{r Average Trip Duration by Month and Vehicle Type}
library(ggplot2)
library(dplyr)
...
ggplot(data = y2020, aes(x = Month2, y = `Trip Duration`, fill = `Vehicle Type`)) + 
  geom_bar(aes(y = `Trip Duration`, x = factor(Month2, levels = level_order_months)), stat = 'summary', fun = mean, position = "dodge") + 
  scale_fill_manual(values = c('red', 'green', 'blue')) + labs(x = 'Month', y = 'Trip Duration') +
  labs(title = "Average Trip Duration by Month and Vehicle Type") + labs(fill = "Vehicle Type")

```

```{r Bar Graph of Average Temperature for Each Month in Austin (2020)}
library(ggplot2)

# Create a data frame with average temperature for each month in 2020
average_temperature_data <- data.frame(
  Month2 = factor(c('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'), 
                  levels = c('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec')),
  Average_Temperature = c(51.6, 56.3, 60.4, 69.5, 76.7, 82.2, 86.5, 89.9, 77, 71.7, 58.9, 55.8)
)

# Create a bar graph to visualize the average temperature for each month
ggplot(data = average_temperature_data, aes(x = Month2, y = Average_Temperature)) +
  geom_bar(stat = 'identity', fill = 'yellow') +
  labs(x = 'Month (2020)', y = 'Average Temperature') +
  labs(title = "Bar Graph of Average Temperature for Each Month in Austin (2020)") +
  scale_x_discrete(limits = levels(average_temperature_data$Month2))
```

```{r Average Trip Duration by Day of the Week and Vehicle Type}
library(ggplot2)
library(dplyr)
...
# Create a new column 'DayOfWeek' with full day names based on numeric values.
y2020 <- y2020 %>%
...
# Define the order of days of the week for plotting.
level_order_dow <- c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday")

# Bar plot to visualize average 'Trip Duration' by 'DayOfWeek' and 'Vehicle Type'.
ggplot(data = y2020, aes(x = factor(DayOfWeek, levels = level_order_dow), y = `Trip Duration`, fill = `Vehicle Type`)) +
  geom_bar(stat = 'summary', fun = mean, position = 'dodge') +
  scale_fill_manual(values = c('red', 'green', 'blue')) +
  labs(x = 'Day of the Week', y = 'Trip Duration') +
  labs(title = 'Average Trip Duration by Day of the Week and Vehicle Type') +
