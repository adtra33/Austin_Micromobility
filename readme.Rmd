...
# Filter the trip data to include only the year 2020
y2020 <- y2020 %>% filter(Year == 2020)
...
# Import monthly mean average temperature data for Austin for 2020
austin_temperature_data <- AustinTemperatureData

# Rename the "Month" column in austin_temperature_data to match the merged_data
names(austin_temperature_data)[names(austin_temperature_data) == "Month"] <- "Month2"

# Convert the "Month" column in the temperature data to the same data type as "Month2"
austin_temperature_data$Month2 <- factor(austin_temperature_data$Month2, levels = levels(y2020$Month2))

# Merge the trip data with temperature data based on the 'Month2' column
merged_data <- y2020 %>% 
  left_join(austin_temperature_data, by = c("Month2" = "Month2"))

# Check the structure of the merged data
str(merged_data)

# Data Cleaning: Convert 'Trip Duration' to numeric
merged_data$`Trip Duration` <- as.numeric(as.character(merged_data$`Trip Duration`))

# Create a bar graph to visualize the relationship between 'Average Temperature' and 'Trip Duration' by 'Vehicle Type' for each month.
ggplot(data = merged_data, aes(x = Month2, y = `Trip Duration`, fill = `Vehicle Type`)) +
  geom_bar(stat = 'identity') +
  labs(x = 'Month (2020)', y = 'Trip Duration') +
  labs(title = "Bar Graph of Trip Duration by Vehicle Type for Each Month") +
  scale_fill_manual(values = c('red', 'green', 'blue')) +
  scale_x_discrete(limits = levels(merged_data$Month2))
