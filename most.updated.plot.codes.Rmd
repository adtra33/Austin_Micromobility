---
title: "project plot and clean dataset"
author: "anh"
date: "2023-11-21"
...
trip_duration_months_h1 <- ggplot(data = y2020, aes(x = Month2, y = Trip.Duration, fill = Vehicle.Type)) + 
  ...
  labs(title = "Average trip duration of each vehicle type throughout the year") + labs(fill = "Vehicle Type")
...
trip_duration_DOW_h1 <- ggplot(data = y2020, aes(x = DOW, y = Trip.Duration, fill = Vehicle.Type)) + 
  ...
  labs(title = "Average trip duration of each vehicle type throughout the week") + labs(fill = "Vehicle Type")
...
```
```{r}
# DISTANCE_AVERAGE TEMP but with 2 variables in group_by():
trip_distance_avg_temp_h2 <- y2020_temp %>% group_by(Average_Temperature) %>% 
  mutate(
    Mean.Trip.Distance.T = mean(log(Trip.Distance), na.rm = T), 
    Mean.Trip.Duration.T =  mean(log(Trip.Duration), na.rm = T)
    ) %>%
  ggplot(aes(x = Average_Temperature, y = Mean.Trip.Distance.T)) +
  geom_point(size = 2, aes(color = Vehicle.Type)) +
  geom_line(aes(group=Vehicle.Type)) +
...
```{r}
# DISTANCE_AVERAGE TEMP but with 2 variables in group_by():
trip_distance_avg_temp_h2 <- y2020_temp %>% group_by(Average_Temperature, Vehicle.Type) %>% 
  mutate(
    Mean.Trip.Distance.T = mean(Trip.Distance, na.rm = T), 
    Mean.Trip.Duration.T =  mean(Trip.Duration, na.rm = T)
    ) %>%
...
trip_duration_avg_temp_h2 <- y2020_temp %>% group_by(Average_Temperature, Vehicle.Type) %>% 
  mutate(
    Mean.Trip.Distance.T = mean(Trip.Distance, na.rm = T), 
    Mean.Trip.Duration.T =  mean(Trip.Duration, na.rm = T)
    ) %>% 
...
```{r}
write.csv(y2020_temp_precip_percent_satisfactory, file = "~/project/y2020_temp_precip_percent_satisfactory.csv", row.names = FALSE)
```

# Scatter plots to show The relation 'Percent Satisfactory' and mean trip distance and duration:
```{r}
# DISTANCE_AVERAGE TEMP:
trip_distance_percent_satisfactory_h4 <- y2020_temp_precip_percent_satisfactory %>% group_by("Percent Satisfactory", Vehicle.Type) %>%
  mutate(
...
```
***WHY trendline looks weird? Intention to group_by("Percent Satisfactory") take the average trip distance at each percent satisfactory, then graph the change in trip distance and percent satisfactory??????????***
...
```{r}
# # Group by Percent Satisfactory and calculate mean trip distance and duration
# compact_y2020_temp_precip_percent_satisfactory <- y2020_temp_precip_percent_satisfactory %>%
#   group_by(`Percent Satisfactory`, Vehicle.Type) %>%
#   summarise(
#     Mean_Trip_Distance = mean(`Trip.Distance`),
#     #Mean_Trip_Duration = mean(`Trip.Duration`),
#     Unique_Council_Districts = unique(`Council.District`)[1]
#   ) %>%
#   #mutate(as.numeric(Unique_Council_Districts)) %>%
#   spread(`Vehicle.Type`, `Mean_Trip_Distance`)
# 
# # Reshape the data to long format for Trip Distance
# compact_y2020_temp_precip_percent_satisfactory_long1 <- compact_y2020_temp_precip_percent_satisfactory %>%
#   pivot_longer(cols = c(bicycle, moped, scooter), names_to = "Vehicle.Type", values_to = "Mean_Trip_Distance")
# 
# head(compact_y2020_temp_precip_percent_satisfactory_long1)
# 
# # Order the levels of Vehicle_Type
# compact_y2020_temp_precip_percent_satisfactory_long1$Vehicle.Type <- factor(compact_y2020_temp_precip_percent_satisfactory_long1$Vehicle.Type, levels = c("scooter", "moped", "bicycle"))
# 
# # Order the data by Mean_Trip_Distance within each Percent Satisfactory level???????????
# compact_y2020_temp_precip_percent_satisfactory_long1 <- compact_y2020_temp_precip_percent_satisfactory_long1 %>%
#   arrange(`Percent Satisfactory`, desc(Mean_Trip_Distance))
# 
# # Convert Council Districts to a factor with proper levels for numerical ordering
# compact_y2020_temp_precip_percent_satisfactory_long1 <- factor(compact_y2020_temp_precip_percent_satisfactory_long1$Unique_Council_Districts, levels = unique(compact_y2020_temp_precip_percent_satisfactory_long1$Unique_Council_Districts))
# 
# head(compact_y2020_temp_precip_percent_satisfactory_long1)
```

```{r}
# # Create the plot with facets for each vehicle type (Trip Distance)
# distance_district <- ggplot(data = compact_y2020_temp_precip_percent_satisfactory_long1, aes(x = reorder(Unique_Council_Districts, `Percent Satisfactory`), y = Mean_Trip_Distance, fill = Vehicle.Type)) +
#   geom_col(position = "dodge", width = 0.5) +
#   scale_fill_manual(values = c('blue', 'green', 'red')) +
#   labs(x = 'Council District', y = 'Mean Trip Distance') +
#   ggtitle("Average Vehicle Duration by Council District (Ordered by Ascending District Percent Satisfactory) and Vehicle Type") +
#   facet_wrap(~Vehicle.Type, scales = "free_y", ncol = 1) +
#   geom_text(aes(label = `Percent Satisfactory`), position = position_dodge(width = 0.03), vjust = 0, color = 'black')
# 
# distance_district
```
...
```{r}
# # Reshape the data to long format for Trip Distance
# compact_y2020_temp_precip_percent_satisfactory_long2 <- compact_y2020_temp_precip_percent_satisfactory %>%
#   pivot_longer(cols = c(bicycle, moped, scooter), names_to = "Vehicle.Type", values_to = "Mean_Trip_Duration")
# 
# head(compact_y2020_temp_precip_percent_satisfactory_long2)
# 
# # Order the levels of Vehicle_Type
# compact_y2020_temp_precip_percent_satisfactory_long2$Vehicle.Type <- factor(compact_y2020_temp_precip_percent_satisfactory_long2$Vehicle.Type, levels = c("scooter", "moped", "bicycle"))
# 
# # Order the data by Mean_Trip_Distance within each Percent Satisfactory level???????????
# compact_y2020_temp_precip_percent_satisfactory_long2 <- compact_y2020_temp_precip_percent_satisfactory_long2 %>%
#   arrange(`Percent Satisfactory`, desc(Mean_Trip_Duration))
# head(compact_y2020_temp_precip_percent_satisfactory_long2)
```

```{r}
# # Create the plot with facets for each vehicle type (Trip Distance)
# duration_district <- ggplot(data = compact_y2020_temp_precip_percent_satisfactory_long2, aes(x = Unique_Council_Districts, y = Mean_Trip_Duration, fill = Vehicle.Type)) +
#   geom_col(position = "dodge", width = 0.5) +
#   scale_fill_manual(values = c('blue', 'green', 'red')) +
#   labs(x = 'Council District', y = 'Mean Trip Distance') +
...
#   ggtitle("Average Vehicle Duration by Council District (Ordered by Ascending District Percent Satisfactory) and Vehicle Type") +
#   facet_wrap(~Vehicle.Type, scales = "free_y", ncol = 1) +
#   # facet_grid(Vehicle.Type ~ ., scales = "free_y", space = "free_y") +
#   geom_text(aes(label = `Percent Satisfactory`), position = position_dodge(width = 0.03), vjust = 0, color = 'black')
...
```{r}
##Temp vs Distance
ggplot(data = y2020_temp, aes(x = Month, y = Average_Temperature, fill = Vehicle.Type)) +
  #create second axis: trip duration
  scale_y_continuous(name = 'Temperature', limits = c(0,90), sec.axis = sec_axis( ~.*10 , name = 'Meters')) +
  #create 1st bar graph for month and precipitation
  geom_bar(alpha = 1, aes(x = factor(Month, level_order_months), y = Average_Temperature, fill = 'blue'), position = position_nudge(x = -0.15), 
           width = 0.3, stat = 'summary', fun = 'mean') +
  #create 2nd bar graph for month and trip distance
  geom_bar(alpha = 1, aes(x = factor(Month, level_order_months), y = Trip.Distance/10, fill = 'red'), position = position_nudge(x = 0.15), 
           width = 0.3, stat = 'summary') +
  labs(title = 'Average Trip Distance in Relation to Average Monthly Temperature') +
  theme(
  ...
  facet_wrap(~Vehicle.Type) + 
  scale_fill_manual(name = 'Variable', values =c('blue'='blue','red'='red'), guide = 'legend', labels = c('Temperature','Distance'))

#temp vs Duration
ggplot(data = y2020_temp, aes(x = Month, y = Precipitation)) +
  #create second axis: trip duration
  scale_y_continuous(name = 'Temperature', limits = c(0,90), sec.axis = sec_axis( ~./10 , name = 'Minutes')) +
  #create 1st bar graph for month and precipitation
  geom_point(alpha = 1, aes(x = factor(Month, level_order_months), y = Average_Temperature, fill = 'blue'), position = position_nudge(x = -0.15), 
           width = 0.3, stat = 'summary', fun = 'mean') +
  geom_line(aes(x = factor(Month, level_order_months), y = Average_Temperature))+
  #create 2nd bar graph for month and trip duration
  geom_bar(alpha = 1, aes(x = factor(Month, level_order_months), y = Trip.Duration*10, fill = 'red'), position = position_nudge(x = 0.15), 
           width = 0.3, stat = 'summary') +
  labs(title = 'Average Trip Duration in Relation to Average Monthly Temperature') + 
  theme(
    axis.text.x = element_text(
      angle = 45,
      hjust = 1,
  ...
  facet_wrap(~Vehicle.Type) + 
  scale_fill_manual(name = 'Variable', values =c('blue'='blue','red'='red'), guide = 'legend', labels = c('Temperature','Duration'))
```

```{r}
#Testing replacing bars with points. 
ggplot(data = y2020_temp, aes(x = Month, y = Precipitation)) +
  #create second axis: trip duration
  scale_y_continuous(name = 'Temperature', limits = c(0,90), sec.axis = sec_axis( ~./10 , name = 'Minutes')) +
  #create 1st bar graph for month and precipitation
